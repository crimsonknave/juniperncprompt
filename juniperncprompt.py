...
# The defaults are what my set up expected, change them if you want
...
parser.add_argument('--password-fields', help="What are the password fields required by your vpn site.  Delimited by commas please (no spaces)", default="password,password#2")
parser.add_argument('-r', '--realm', help="What realm are we using.  This will be a hidden field in the web form on the vpn site", default="Internal Users")
...
parser.add_argument('--logout-path', help="The path to the logout call, so we don't leave sessions trailing behind us.", default="/dana-na/auth/logout.cgi")
